...
class _CallTracerFinishBreakpoint(gdb.FinishBreakpoint):
    def __init__(self, name, stack):
        super(_CallTracerFinishBreakpoint, self).__init__(self,gdb.newest_frame())
...
class _CallTracerBreakpoint(gdb.Breakpoint):
    def __init__(self, spec, name, stack):
        super(_CallTracerBreakpoint, self).__init__(spec, gdb.BP_BREAKPOINT,
                                                 internal = False)
	self._stack_ptr=stack
	self._name=str(name)
    def stop(self):
	...
	print (" " * (len(self._stack_ptr)))+"=>"+self._name
	try:
		_CallTracerFinishBreakpoint(self.name, self._stack_ptr)
	except:
		print "uh? cant put break on "+self.name
        return False
...
class _PrepareCallTracer(gdb.Command):
    ...
    def __init__(self):
       super(_PrepareCallTracer, self).__init__('prepcalltracer',gdb.COMMAND_OBSCURE)
       self._stack=[]
    def _retrive_ptrs(self):
	info=gdb.execute("info break",False, True)
	ptrs=[]
	info_lines=info.splitlines()
	break_infos=gdb.breakpoints()
	for idx in range(0,len(info_lines[2:])):
		tokens=info_lines[idx].split()
		if len(tokens) > 5:
			item={'addr': tokens[4],
			      'name': break_infos[idx].location}
			ptrs.append(item)
	return ptrs
	...
	for item in break_info:
		_CallTracerBreakpoint(r'*'+item['addr'],
			item['name'],self._stack)
	print "prepare done!"
