...
\subsection{wayland}
 waylandとは、Kristian Hogsbergさんが中心となって作っているディスプレイサーバーのプロトコルのことです。 

 Unixで有名なディスプレイサーバーのプロトコルとしては、Xプロトコルがありますが、1984年頃のハードの状況を元に設計されているため、内部がいろいろつぎはぎだったり、コンポジット型の画面制御をするのがそもそも苦手だったりします。今ではそれでもなんとか改造が続いているようですが、元々の設計やら、今までつぎはぎしてきた使われないかもしれない機能の互換性を保ち続ける労力も馬鹿にならないため、いっそのことシンプルに作り直したディスプレイサーバーが必要となってきました。waylandはこれらの欠点を補うものとなります。

\subsection{waylandを動かしてみる}
